...
export class CCompiler extends BaseCompiler {
  ...
  constructor(options: CompilerOptions) {
    super(options);
    const baseName = this.options.fileName.replace(/\.(c)$/i, '');
  ...
  protected async preprocess(): Promise<boolean> {
    ...
    try {
      ...
      const preprocessCmd = `gcc -E ${sourcePath} -o ${sourcePath}.i`;
...
export function createCCompiler(options: CompilerOptions): CCompiler {
  return new CCompiler({
    ...options,
    fileName: options.fileName.endsWith('.c') ? options.fileName : `${options.fileName}.c`
  });
}
